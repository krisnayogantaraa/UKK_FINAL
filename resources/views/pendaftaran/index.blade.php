...
@section('content')
<p class="text-5xl mb-3 dark:text-white">Daftar Jurusan</p>
<a href="{{ route('major.create') }}">
    <button type="button" class="focus:outline-none text-white font-medium rounded-lg text-lg px-5 py-2.5 me-2 mb-2 bg-green-600 hover:bg-green-700 focus:ring-green-800 w-56">Tambah Jurusan Baru</button>
</a>

<div>
    <form class="w-48 mb-3 float-right" action="/major" method="get">
        <div class="flex">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                ...
                <tr class=" odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                    <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $major->kd_jurusan }}</td>
                    <td class="px-6 py-4">{{ $major->nama_jurusan }}</td>
                    <td class="px-6 py-4">{{ $major->link_group }}</td>
                    <td class="px-6 py-4">
                        <div class="text-container w-20">
                            {{ $major->visi }}
                        </div>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">{{ $major->total_biaya }}</td>
                    <td class="px-6 py-4">
                        <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('major.destroy', $major->id) }}" method="POST">
